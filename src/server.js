...
export function llmsTxt(router) {
  ...
  const lines = [
    ...
    '',
    '    curl -s -H "$AUTH" http://127.0.0.1:8080/api/events/EVENT/submissions?status=pending',
    '    curl -s -H "$AUTH" -X POST http://127.0.0.1:8080/api/events/EVENT/submissions/SESS-3/decide \\',
    '         -H "content-type: application/json" -d \'{"decision":"accept"}\'',
    '    # nothing has been sent yet. This is what sends it:',
    '    curl -s -H "$AUTH" -X POST http://127.0.0.1:8080/api/events/EVENT/notify \\',
    ...
    '',
    '    curl -s -H "$AUTH" -X POST http://127.0.0.1:8080/e/EVENT/forms/FORM/routing \\',
    ...
    '    curl -s -H "$AUTH" http://127.0.0.1:8080/e/EVENT/forms/FORM',
    '',
    '`plan` is a review round slug (GET /e/EVENT/evaluation), `track` a track slug.',
    ...
    '',
    '    curl -s -H "$AUTH" http://127.0.0.1:8080/api/events/EVENT/agenda      # see what has no slot',
    '    curl -s -H "$AUTH" -X POST http://127.0.0.1:8080/e/EVENT/agenda/autoschedule',
    '    # or place one by hand:',
    '    curl -s -H "$AUTH" -X POST http://127.0.0.1:8080/e/EVENT/submissions/SESS-3/schedule \\',
    ...
    '',
    '    curl -s -H "$AUTH" -X POST http://127.0.0.1:8080/e/EVENT/submissions/SESS-3/content \\',
    '         --data "content_status=approved"',
    '    curl -s -H "$AUTH" -X POST http://127.0.0.1:8080/e/EVENT/agenda/publish',
    '',
    '### Give somebody a feed of the programme for their own website',
    '',
    '    curl -s -H "$AUTH" -X POST http://127.0.0.1:8080/e/EVENT/embeds \\',
    '         --data "name=Agenda&feed=agenda&format=json"',
    '    # then it is served, cross-origin, at:',
    '    curl -s http://127.0.0.1:8080/embed/EVENT/agenda.json',
    '',
    'Formats are html, json, xml, ics. Feeds are agenda, session_list,',
    'schedule_itinerary, speaker_list, speaker_gallery. Note /api/... is organizer',
    'data with no CORS header; it is not a public feed.',
    '',
    '### Email a group about something other than a decision',
    '',
    '    curl -s -H "$AUTH" "http://127.0.0.1:8080/e/EVENT/mail?audience=outstanding-tasks"',
    '    curl -s -H "$AUTH" -X POST http://127.0.0.1:8080/e/EVENT/mail \\',
    ...
    '',
    '    curl -s -H "$AUTH" -X POST http://127.0.0.1:8080/e/EVENT/tasks/definitions \\',
    '         --data "title=Upload your slides&applies_to=submission&requirement=file" \\',
    '         --data "due_at=2027-04-30&assign_when=on_accept"',
    '    curl -s -H "$AUTH" http://127.0.0.1:8080/e/EVENT/tasks/definitions   # what is being asked for',
    '    curl -s -H "$AUTH" http://127.0.0.1:8080/api/events/EVENT/tasks      # who still owes what',
    '',
    ...
    '',
    '    curl -s -H "$AUTH" -X POST http://127.0.0.1:8080/e/EVENT/tasks/definitions/TASK/retire',
    '',
    '### Collect what speakers have uploaded',
    '',
    '    curl -s -H "$AUTH" -o files.zip "http://127.0.0.1:8080/e/EVENT/files.zip?group=speaker&task=upload-slides"',
    '',
