...
export function llmsTxt(router, { brief = false } = {}) {
  ...
  const lines = [
    ...
    '         --data "field=track&operator=equals&value=TRACK-SLUG" \\',
    '         --data "plan=ROUND-SLUG&track=TRACK-SLUG&reviewers=2"',
    '    # what the rules have actually done, and why, is on the form page:',
    `    curl -s -H "$AUTH" ${PUBLIC_ORIGIN}/e/EVENT/forms/FORM`,
    '',
    'EVENT, FORM, TRACK-SLUG and ROUND-SLUG are all yours to fill in, and all four',
    'are listed by GET /api/events/EVENT -- which is the one call to make before',
    'any of this. Values compare against an answer\'s slug, not its label.',
